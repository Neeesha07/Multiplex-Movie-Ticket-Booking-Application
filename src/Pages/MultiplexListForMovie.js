...
function MultiplexListForMovie() {
    ...
    useEffect(() => {
        const fetchMultiplexes = async () => {
            try {
                // const response = await fetch('http://win10-2-186:8888/multiplex-ms/getMultiplex/1'); // Ensure you use http://
                const response = await fetch(`http://win10-2-186:8888/multiplex-ms/allMultiplexesByMovieName/${encodeURIComponent(movieName)}`); // Ensure you use http://
    ...
    const handleTimeslotClick = (timeslot, screeningId) => {
        ...
        navigate(`/seats/${movieName}/${screeningId}`,{ state: { movie } });
    };
    ...
    const multiplexesWithScreenings = multiplexes.filter(multiplex => 
        multiplex.movies.some(movie => 
            ...
            movie.screenings.some(screening => screening.availableSeats.length > 0 && new Date(screening.timeSlot).toISOString().split('T')[0] === selectedDate)
        )
    );
            ...
            <div className="container bg-white rounded shadow p-4" style={{ opacity: 0.9 }}>
                ...
                <div className="text-center mb-4">
                    ...
                    <h2 className="text-black" style={{ fontSize: '1.5rem' }}>
                        on <span className="font-weight-bold">{new Date(selectedDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
                    </h2>
                </div>
                    ...
                    {multiplexes.map(multiplex => {
                        const availableScreenings = multiplex.movies.flatMap(movie =>
                            movie.movieName === movieName ? movie.screenings.filter(screening => {
                                ...
                                return screening.availableSeats.length > 0 && screeningDate === selectedDate;
                            }) : []
                        );
