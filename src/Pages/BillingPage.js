...
const BillingPage = () => {
  ...
  const {screenId }= useParams(); // hard-coded for demonstration
  const location=useLocation();
  const { selectedSeats } = location.state || {} ;// hard-coded for demonstration
   // hard-coded booker ID for demonstration
  
  const bookerId = ticketBooker.bookerId;
  ...
  useEffect(() => {
    const manageTicket = async () => {
      ...
      if (ticketId) {
        try {
          await axios.post(`http://win10-2-186:8888/ticketBooker/cancelPayment/${ticketId}`);
      ...
      try {
        const response = await axios.post(`http://win10-2-186:8888/multiplex-ms/createNewTicket/${bookerId}`, ticketRequest);
  ...
  const showDiscounts = async () => {
    if (ticket) {
      try {
        const response = await axios.get(`http://win10-2-186:8888/ticketBooker/getApplicableDiscounts/${ticket.totalAmount}`);
  ...
  const applyDiscount = async () => {
    if (selectedDiscount !== null && ticket) {
      ...
      try {
        const response = await axios.post(`http://win10-2-186:8888/ticketBooker/applyDiscount/${ticket.ticketId}`, {
